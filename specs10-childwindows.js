...
//			browser.sleep(2000);
			browser.getTitle().then(function(title){
				console.log("We are in this page: " + title);
			})
		})
	})
	
	it("Switching to the child-window", function(){
		element(by.css("a[ng-href='/place/GB/London/River Island']")).click().then(function(){
			browser.getTitle().then(function(title){
				console.log("We clicked the link but we are still in: " + title);
			})
			browser.getAllWindowHandles().then(function(handles) {
				browser.switchTo().window(handles[0]).then(function() {
					browser.sleep(2000);
					browser.getTitle().then(function(title){
						console.log("We switched to this page: " + title);
					})
				})
			})
		})
